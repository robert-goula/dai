...
export type InstallState = {
  /** Docsets being installed or updated, from any client (app, CLI). */
...
export function useDaiEvents(onOpen: (page: Page) => void): InstallState {
  ...
  const [state, setState] = useState<InstallState>({ installing: new Set(), errors: new Map() });
    ...
    const unlisten = listen<DaiEvent>(DAI_EVENT, ({ payload: e }) => {
      switch (e.type) {
        ...
        case "install_started":
          setState((s) => ({
            installing: new Set(s.installing).add(e.id),
